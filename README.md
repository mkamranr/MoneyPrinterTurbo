...
> [!NOTE]
> **This repository is a fork of [harry0703/MoneyPrinterTurbo](https://github.com/harry0703/MoneyPrinterTurbo).**
>
> All credit for the original project belongs to [harry0703](https://github.com/harry0703) and the upstream
> [contributors](https://github.com/harry0703/MoneyPrinterTurbo/graphs/contributors). This fork follows upstream
> and adds support for running the script writer and the voiceover on self-hosted models — see
> [Local Models](#local-models-). Distributed under the same [MIT License](LICENSE) as the original.
...
- [x] Supports **Edge TTS**, **Azure Speech**, **SiliconFlow**, **Google Gemini**, **Xiaomi MiMo**, **ElevenLabs**, **Chatterbox**, **Fish Audio**, and any **locally hosted OpenAI-compatible TTS** server, all with real-time previews
...
- [x] Supports leading model providers including **Kimi / Moonshot AI**, **OpenAI**, **Anthropic Claude**, **Google Gemini**, **DeepSeek**, **Alibaba Cloud Qwen**, **Microsoft Azure OpenAI**, **ByteDance VolcEngine Ark**, **xAI Grok**, **MiniMax**, and **Xiaomi MiMo**, plus unified gateways, aggregators, and local runtimes such as **Cloudflare AI Gateway**, **Alibaba ModelScope**, **AIHubMix**, **AIML API**, **EvoLink**, **vLLM**, **Ollama**, **OneAPI**, **LiteLLM**, **Groq**, and **Pollinations AI**
- [x] Runs **entirely on your own hardware** when paired with local models: [vLLM](https://docs.vllm.ai) or Ollama for the script, a local OpenAI-compatible TTS server for the narration, and your own footage — no cloud API key required
...
Start a server for the model you want to use:

```shell
vllm serve Qwen/Qwen2.5-7B-Instruct
```

In the WebUI open **Settings → LLM Settings** and select **vLLM**:

- **API Key** — leave it blank. Fill it in only if you started the server with
  `vllm serve --api-key <token>`.
- **Base Url** — prefilled with `http://localhost:8000/v1`. Inside Docker the host address is
  resolved automatically, so `http://host.docker.internal:8000/v1` is used instead.
- **Model Name** — the model the server is hosting, for example `Qwen/Qwen2.5-7B-Instruct`.

Use **Test LLM Connection** to confirm the setup before you generate anything. **Ollama** is
configured the same way, on port `11434`.

The equivalent `config.toml` entries:
...
Click **Play Voice** for a short sample before committing to a full generation.
...
> see [Subtitle Generation](#subtitle-generation-).

## Voice Synthesis 🗣

The default provider is the free **Edge TTS**, shown as **Azure TTS V1** in the WebUI. MoneyPrinterTurbo also supports **Azure TTS V2**, **SiliconFlow TTS**, **Google Gemini TTS**, **Xiaomi MiMo TTS**, **ElevenLabs TTS**, self-hosted **Chatterbox TTS**, **Fish Audio TTS**, **Local TTS (OpenAI-compatible)** for any server you run yourself (see [Local Models](#local-models-)), and a no-voice mode.
...
MoneyPrinterTurbo was created by [harry0703](https://github.com/harry0703). This repository is a
...
MIT, unchanged from upstream: Copyright (c) 2024 Harry. Click to view the
